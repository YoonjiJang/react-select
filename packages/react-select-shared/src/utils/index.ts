...
export const cleanCommonProps = <
  ...
  AdditionalProps
>(
  props: Partial<BaseCommonPropsAndClassName<Option, IsMulti, Group>> &
    AdditionalProps
) => {
  //className
  const {
...
>(
  props: Pick<
    BaseCommonPropsAndClassName<Option, IsMulti, Group>,
    'cx' | 'getClassNames' | 'className'
  >,
  name: keyof StylesProps<Option, IsMulti, Group>,
  classNamesState?: ClassNamesState
) => {
  const { cx, getClassNames, className } = props;
  return {
